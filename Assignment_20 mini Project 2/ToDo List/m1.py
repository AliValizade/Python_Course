...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        loader = QUiLoader()
        self.ui = loader.load('todo.ui', None)
        self.ui.show()
    ...
    def priority(self):
        id = self.sender().objectName().split('_')[-1]
        priority = d1.get_detail(id)[0][5]
        if priority==1:
            d1.update_priority(id, 0)
        else:
            d1.update_priority(id, 1)
        self.clearUI_tasks()
        self.readFromDataBase()
    ...
    def addNewTaskToDataBase(self):
        ...
        d1.add(id, title, description, time)
